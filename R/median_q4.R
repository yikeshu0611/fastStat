...
# median_q4(df,c('g1','g2'))
# importFrom stats quantile
median_q4<-function(x,group,wide=TRUE,round=2){
    fun_temp<-function(x,round){
        paste0(digital(median(x,na.rm = TRUE),round),
               '(',
               digital(quantile(x,0.25,na.rm=TRUE),round),
               ',',
               digital(quantile(x,0.75,na.rm=TRUE),round),
               ')'
        )
    }
